        ...
        for (let i = 0; i < arr[iHour].length - 1; i++) {
            ...
            if(iHour != 0){
                if(arr[iHour][i][1] > arr[iHour - 1][i][1]){
                     text += '<td class="backGreen">' + arr[iHour][i][2] + '</td><td>';
                }
                else if(arr[iHour][i][1] < arr[iHour - 1][i][1]){
                     text += '<td class="backOrange">' + arr[iHour][i][2] + '</td><td>';
                }
                else{
            ...
            // якщо кількысть блоків можливо перевести в число
                ...
                if (arr[iHour][i][2] == 0) {
                    ...
                    text += arr[iHour][i][1].replace('.', ',') + '</td><td>';
                }
            }
            else {
                if (arr[iHour][i][2] == 0) {
                // якщо потужність 0 то і кількість блоків вписуємо 0
                    ...
                    text += (Number(sp[1]) + Number(sp[0])).toString().replace('.', ',') + '</td><td>';
                }
            }
            if (arr[iHour][i][2] == 0) {
                text += 0 + '</td>';
